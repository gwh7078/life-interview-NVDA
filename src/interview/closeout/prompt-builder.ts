...
function compactPromptReferences(context: StoryCloseoutContext): CompactPromptReferences {
  ...
  const messages = context.transcript.map((message) => {
    const messageId = message.role === 'user' ? `u${++userIndex}` : `a${++assistantIndex}`;
    if (message.role === 'user') sourceMessageIds.set(messageId, message.message_id);
    return { message_id: messageId, role: message.role, text: message.text };
  });
...
export function buildStoryCloseoutPrompt(
  ...
  if (context.mode === 'continue' && context.currentStory) {
    const system = [
      ...
      'source_message_ids 只能引用输入 Transcript 中 role=user 的 u# message_id，且必须直接支持对应摘要或 Agent Memory 更新；不能引用 assistant 的 a#、编造 ID 或引用其它访谈。对 add/correct/refine，new_text 必须保留引用消息中的具体事实锚点（如人名、地点、时间、事件动作或关键原话），不能拿一条真实但无关的 message_id 给新增事实背书。',
  ...
  const system = [
    ...
    'source_message_ids 只能引用输入 Transcript 中 role=user 的 u# message_id，且必须直接支持 Story 标题、摘要或 Agent Memory；不能引用 assistant 的 a#、编造 ID 或引用其它访谈。',
