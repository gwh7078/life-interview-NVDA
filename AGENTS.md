...
- On macOS, keep provider API keys in the user's login Keychain; setup may materialize only the required `STEPFUN_API_KEY`, `BAILIAN_API_KEY`, and `CLOSEOUT_API_KEY` into the current Worktree's ignored `.env` with mode 0600. Never copy `.env` into another Worktree, add it to `.worktreeinclude`, commit it, or print its values.
- `STEPFUN_API_KEY` is for the Step-Audio 2 Mini realtime voice model. `BAILIAN_API_KEY` and `CLOSEOUT_API_KEY` are separate text-model credentials and must not be reused as realtime credentials.
